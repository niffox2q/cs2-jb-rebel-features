#include "jb_rebel_features.h"
#include <random>
#include <cstdio>
#include <algorithm>

jb_rebel_features g_jb_rebel_features;
PLUGIN_EXPOSE(jb_rebel_features, g_jb_rebel_features);
...
void LoadConfig() {
    ...
    const char* path = "addons/configs/Jailbreak/rebel_features.ini";
...
void StartupServer() {
    ...
    jailbreak_api->OnRebelPrisoner(g_PLID,[](int iSlot){
        auto pController = CCSPlayerController::FromSlot(iSlot);
        if (!pController) return;
...
bool jb_rebel_features::Load(PluginId id, ISmmAPI* ismm, char* error, size_t maxlen, bool late) {
    PLUGIN_SAVEVARS();
...
void jb_rebel_features::AllPluginsLoaded() {
    int ret;
...
bool jb_rebel_features::Unload(char* error, size_t maxlen) {
...
const char* jb_rebel_features::GetAuthor() { return "niffox"; }
const char* jb_rebel_features::GetDate() { return __DATE__; }
const char* jb_rebel_features::GetDescription() { return "[JB] Rebel Features"; }
const char* jb_rebel_features::GetLicense() { return "Private"; }
